...
(provide chop                        ; (chop l n)
	 ...
	 string-extract              ; (string-extract str field [delim #f])
	 echo                        ; (echo str1 str2 ...)
	 )
...
;; returns a trimmed field from a given string
(define (string-extract str field [delim #f])
  (define parts
    (map string-trim (if (not delim)
                         (string-split str)
                         (string-split str delim))))
  (if (pair? parts)
      (list-ref parts field)
      "?"))
