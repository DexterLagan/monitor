...
;; returns the best guess for the config file location
(define (get-config-file)
  (string-append (path->string (find-system-path 'exec-file)) ".conf"))

;; check config file exists and read, clean and check config file
(define (load-config filename section-length)
  (define config-file (if (file-exists? filename) 
			  filename
			  (get-config-file)))
...
(define (process-config-items proc config-lst [optional-param-str1 ""] [optional-param-str2 ""])
  (let ((result (andpmap (λ (c) (proc c optional-param-str1 optional-param-str2)) config-lst)))
    (unless result 
      (die (string-append "Error executing configurations: " (~a config-lst))))))
